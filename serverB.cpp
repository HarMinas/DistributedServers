...
int main()
{
	...
	string fileName = "backendB.txt"; //The file of from which the server will pull data.
	ifstream dictionary;	//The input file stream that will get data from the file.

	int serverSocket; //the socket file descriptor.
	const int serverPort = 22195; //port number of the server
...
//Binding the socket to the server address.
	...
	cout << "Server B is up and running using UDP on port " << serverPort << endl;
...
/*An infinite loop that receives a request through a server UDP socket, performs that requested operations
   and sends the results to the AWS server.*/
	while(true){
		...
		cout << "Server B received input <" << input << "> and operation <" << command << ">" << endl;
		...
		cout << "The Server B finished sending the output to the AWS." << endl;
...
void performOperation(string operation, string input, vector<string> * results, ifstream& dict, string fileName)
{
	...
	{ 
		...
		{
			cout <<"The Server B has found <" << 1 << "> match and <" << temp.size() << "> similar words." << endl; //there is a match for definition.
		}else
		{	
			cout <<"The Server B has found <" << 0 << "> match and <" << temp.size() << "> similar words." << endl;  //There was no match for definition.
		}
	}
	else if (operation == prefixKey || operation == suffixKey) //perform prefix or suffix operations.
	{
		...
		cout << "The Server B has found <" << results->size() - 2 <<"> matches" <<endl;
	}
