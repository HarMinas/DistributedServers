...
int main()
{
	...
	string fileName = "backendA.txt"; //The file of from which the server will pull data.
	ifstream dictionary;	//The input file stream that will get data from the file.

	int serverSocket; //the socket file descriptor.
	const int serverPort = 21195; //port number of the server
...
//Binding the socket to the server address.
	...
	cout << "Server A is up and running using UDP on port " << serverPort << endl;
...
/*An infinite loop that receives a request through a server UDP socket, performs that requested operations
   and sends the results to the AWS server.*/
	while(true){
		...
		cout << "Server A received input <" << input << "> and operation <" << command << ">" << endl;
		...
		cout << "The Server A finished sending the output to the AWS." << endl;
...
void performOperation(string operation, string input, vector<string> * results, ifstream& dict, string fileName)
{
	...
	{ 
		...
		{
			cout <<"The Server A has found <" << 1 << "> match and <" << temp.size() << "> similar words." << endl; //there is a match for definition.
		}else
		{	
			cout <<"The Server A has found <" << 0 << "> match and <" << temp.size() << "> similar words." << endl;  //There was no match for definition.
		}
	}
	else if (operation == prefixKey || operation == suffixKey) //perform prefix operation
	{
		...
		cout << "The Server A has found <" << results->size() - 2 <<"> matches" <<endl;
	}
